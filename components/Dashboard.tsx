...
const Dashboard: React.FC = () => {
  ...
  const fetchBIData = useCallback(async () => {
    ...
    // REVISED SQL: Using PLUCode instead of Barcode
    ...
    try {
      ...
      if (brief) {
        ...
        setAiBrief(`Local Insight: ${opYear} revenue is R${mRev.toLocaleString()}. AI Narrative Engine offline.`);
      }
    } finally {
  ...
  if (!stats) return (
    <div className="flex flex-col items-center justify-center h-full space-y-4">
      ...
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 italic">Reading Ultisales Schema...</p>
    </div>
  );
